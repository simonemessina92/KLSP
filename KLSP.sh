...
echo -e "\n\033[1;44m     🚀 Kiloview KLSP Installer Starting...     \033[0m\n"
sleep 1
...
install() {
    ...
    stream_server_ip=$(curl -s ifconfig.me || curl -s ipinfo.io/ip)
    echo
    read -p "Detected public IP address is: $stream_server_ip. Is this correct? [Y/n] " CONFIRM_IP
    if [ "$CONFIRM_IP" = "n" ] || [ "$CONFIRM_IP" = "N" ]; then
        read -p "Please enter the correct IP: " stream_server_ip
    fi
    ...
    if ! command -v avahi-daemon > /dev/null; then
        echo "[INFO] Installing avahi-daemon..."
        if command -v apt > /dev/null; then
            sudo apt update && sudo apt install -y avahi-daemon
        elif command -v yum > /dev/null; then
            sudo yum install -y avahi-daemon
        else
            echo "[ERROR] No supported package manager found. Please install avahi-daemon manually."
            exit 1
        fi
    fi
